...
export const getTokenFromAuthorizationCookie = ({ headers }: { headers: Record<string, any> }): string | null => {
  ...
  const [_, authorization] = new RegExp(/ (authorization=[a-zA-Z0-9\-_.]+);/i).exec(` ${cookies};`) ?? [];
  if (!authorization) return null; // auth cookie not found
  const potentiallyAToken = authorization.replace(/^authorization=/i, '');
