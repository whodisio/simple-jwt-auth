 ...
 */
export const getSignedClaims = async <C extends MinimalTokenClaims>({ token, publicKey }: { token: string; publicKey: string }): Promise<C> => {
  ...
  await verifyTokenSignature({ token, publicKey });
